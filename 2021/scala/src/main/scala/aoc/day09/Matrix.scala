...
case class Matrix(input: Vector[Vector[Int]]):
  ...
  def apply(coords: (Int, Int)): Int = input(coords._1)(coords._2)
  ...
  def indices = (0 until height).flatMap(row => (0 until width).map(col => (row, col))).toVector
  ...
  def surround(row: Int, col: Int) = 
    ...
    Vector(up, down, left, right).filter(!indexOutside(_, _))
